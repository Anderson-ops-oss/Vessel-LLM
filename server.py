...
# import requests # Removed requests as we are using local model
import re
import html
import logging
import shutil
...
from core.rag_trainer import RAGSystem, Qwen3VLEmbedding, Qwen3VLReranker
from typing import Iterator

# Load environment variables
load_dotenv()

# Setup directories and environment for model caching
...
def initialize_model() -> bool:
    """
    Initialize and load the local LLM model with quantization.
    Returns:
        bool: True if model loaded successfully, False otherwise.
    """
    global model, processor, MODEL_LOADING_ERROR
    try:
        model_name = os.getenv("LLM_MODEL", "Qwen/Qwen3-VL-8B-Thinking")
...
def initialize_rag_systems():
    """
    Initialize RAG systems with shared models.
    """
    global rag_systems, RAG_AVAILABLE, SHARED_EMBED_MODEL, SHARED_RERANKER, BASE_MODEL_DIR
    
    # Set up RAG model directory
    os.makedirs(BASE_MODEL_DIR, exist_ok=True)
    
    try:
        print("Initializing shared RAG models (Embedding & Reranker)...")
        device = "cuda" if torch.cuda.is_available() else "cpu"
        
        # Get model names from env
        embed_name = os.getenv("EMBEDDING_MODEL", "Qwen/Qwen3-VL-Embedding-2B")
        reranker_name = os.getenv("RERANKER_MODEL", "Qwen/Qwen3-VL-Reranker-2B")
        
        # Load Embedding Model
        SHARED_EMBED_MODEL = Qwen3VLEmbedding(model_name=embed_name, device=device, cache_folder=CACHE_DIR)
        SHARED_RERANKER = Qwen3VLReranker(model_name=reranker_name, device=device, cache_folder=CACHE_DIR)
        
        print("Shared RAG models initialized.")

        #Load RAG Systems using shared models
        rag_systems = {}
        for model_dir in os.listdir(BASE_MODEL_DIR):
            ...
            if os.path.isdir(model_path) and os.path.exists(config_path):
                try:
                    rag_systems[model_dir] = RAGSystem(
...
def log_user_question(question: str) -> None:
    ...
    Args:
        question (str): The user question to log.
    Returns:
        None, the user question is logged to a file.
    """
    try:
        # Log the user question with a timestamp
...
def call_llm_local(messages: list, temperature: float = 0.7, max_tokens: int = 2048):
    """
    Local LLM generation with streaming support using Transformers.
    Args:
    ...
    if model is None or processor is None:
        yield f"Error: Model not loaded. {MODEL_LOADING_ERROR or 'Unknown error'}"
        return
    ...
    try:
        ...
        # Process vision info
        image_inputs, video_inputs, image_grid_thw_h = process_vision_info(messages)
        inputs = processor(
            text=[text],
            images=image_inputs,
            videos=video_inputs,
        ...
        logger.error(f"Error in call_llm_local: {str(e)}")
        yield f"Error during generation: {str(e)}"


def build_conversation_context(session_id: str, new_message: str, system_prompt: str = "") -> list:
    """
    Build conversation context with history. 
    Args:
        session_id (str): The session ID for the conversation.
        new_message (str): The new user message to add.
        system_prompt (str): Optional system prompt to include.
    Returns:
        list: List of message dicts including history and new message.   
    """
...
def update_conversation_context(session_id: str, user_message: str, ai_response: str) -> None:
    ...
    Args:
        ...
        ai_response (str): The AI response to add.
    Returns:
        None, the conversation context is updated in place.
    """
    # Get the current conversation context
    context = conversation_contexts[session_id]
...
def generate_streaming_response(messages: list, session_id: str, question: str, temperature: float) -> Iterator[str]:
    """
    Shared function to generate streaming responses for all endpoints
    Args:
        messages (list): List of message dicts for the conversation.
        session_id (str): The session ID for the conversation.
        question (str): The user question to log and update context.
        temperature (float): Sampling temperature for generation.
    ...
    try:
        # Call the local LLM
        generator = call_llm_local(messages, temperature=temperature)
...
def extract_file_content(file_path: str, file_extension: str) -> str:
    ...
    Args: 
        ...
        file_extension (str): The file extension (e.g., .docx, .pdf).
    Returns:
...
def training_progress_stream() -> Response:
...
def upload_and_ask():
    """
    Handle questions with optional multiple file uploads using multimodal or text-based response.
    Returns:
        Response: Streaming response with AI-generated content.
    """
    try:
        ...
        # Combined message content (multimodal list)
        multimodal_content = []
        # Text content from extracted files
        extracted_text_blocks = []

        processor_tool = DocumentProcessor(UPLOADS_DIR)
        ...
        if files:
            for file in files:
                ...
                if file_extension not in ['.docx', '.pdf', '.xlsx', '.xls', '.txt', '.png', '.jpg', '.jpeg', '.webp']:
                ...
                if file_extension in ['.png', '.jpg', '.jpeg', '.webp']:
                    ...
                    multimodal_content.append({"type": "image", "image": Image.open(file_path).convert("RGB")})
                
                # Handle PDFs (Multimodal for <= 5 pages, else text)
                elif file_extension == '.pdf':
                    page_count = processor_tool.get_pdf_page_count(file_path)
                    if 0 < page_count <= 5:
                        logger.info(f"PDF {file.filename} is {page_count} pages. Using multimodal path.")
                        pdf_images = processor_tool.convert_pdf_to_images(file_path, max_pages=5)
                        for img in pdf_images:
                            multimodal_content.append({"type": "image", "image": img})
                    else:
                        logger.info(f"PDF {file.filename} is {page_count} pages. Using text extraction path.")
                        file_content = extract_file_content(file_path, file_extension)
                        extracted_text_blocks.append(f"File: {file.filename}\n{file_content}")
                
                # Handle other text formats (Word, Excel, Txt)
                else:
                    file_content = extract_file_content(file_path, file_extension)
                    extracted_text_blocks.append(f"File: {file.filename}\n{file_content}")
                ...
                # Clean up
                # os.remove(file_path)
            logger.info(f"Processed {len(files)} files. Multimodal items: {len(multimodal_content)}")
            
        # Build the final prompt
        text_context = "\n\n".join(extracted_text_blocks)
        prompt_text = f"Question: {question}"
        if text_context:
            prompt_text += f"\n\nContext from longer documents:\n{text_context}"
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a helpful assistant that MUST answer questions based ONLY on the provided content. "
                    "You have been provided with images of some documents and text extracted from others. "
                    "If the content does not contain enough information, say so."
                )
            },
            {
                "role": "user",
        ...
        """Note: I am currently NOT using conversation context for /upload_and_ask 
            to simplify multimodal management, but I will add it later.
        """
        ...
        logger.error(f"Error in /upload_and_ask: {str(e)}", exc_info=True)
...
def get_rag_models():
    ...
    Returns:
        Response: JSON response with list of RAG models.
    """
    try:
...
def process_folder_for_rag(upload_dir: str) -> tuple:
    ...
    Args:
        upload_dir (str): The directory containing uploaded files.
    Returns:
        tuple: (processed_dir (str), file_count (int))
    """
...
def upload_folder_for_rag():
    ...
    try:
        ...
        try:
            ...
            # Use shared models for training
            new_rag = RAGSystem(
                processed_texts_dir=processed_dir, 
...
def rename_rag_model():
    """
    Rename the RAG model.
    Returns:
        Response: JSON response with rename status.
    """
    try:
        data = request.get_json()
...
def health_check():
    ...
    Returns:
    Response: JSON response with server status.
    """
    api_type = "Local HuggingFace Model"
    global model
...
def test_llm():
    ...
    Returns:
    Response: JSON response with test result.
    """
    global model, processor
    try:
        if model is None or processor is None:
             return jsonify({"status": "error", "message": "Model or processor not loaded"}), 500
        ...
        # Simple non-streaming generation for test
        text = processor.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=True
        )
...
def clear_context():
    ...
    Returns:
    Response: JSON response with clear status.
    """
    try:
...
if __name__ == '__main__':
    try:
        ...
        if not success:
            logger.warning(f"LLM Model failed to load: {MODEL_LOADING_ERROR}")
            print(f"Warning: Model failed to load. API endpoints requiring LLM will fail.")
        else:
             logger.info("LLM Model initialized successfully")

        # Initialize RAG systems (Shared Load)
