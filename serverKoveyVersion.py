        ...
        def generate():
            ...
            try:
                ...
                if ai_response_content:
                    # Process thinking and response parts
...
def clear_context():
    ...
    try:
        # Try to get session_id from JSON body first, then from Flask session
        session_id = None
        if request.json and 'session_id' in request.json:
...
def get_context():
    ...
    try:
        # Try to get session_id from URL parameter first, then from Flask session
        session_id = request.args.get('session_id')
        if not session_id:
            session_id = get_or_create_session_id()
            
        context = list(conversation_contexts[session_id])
